    ...
    return (
        <div className='fixed min-w-1/6 h-full p-5 drop-shadow bg-indigo-600 z-10'>
            ...
            <ul className='flex flex-col h-full text-white text-base'>
                <li className={`p-3 mb-1 hover:bg-indigo-700 rounded-lg cursor-pointer ${adminMenu.name === 'list' && 'bg-indigo-700'}`} onClick={() => handleMenuName('list')}>
                    <a href="/admin/products/b" className='flex items-center'>
                        <RiListCheck className='mr-2'/>상품 리스트
                    </a>
                </li>
                <li className={`p-3 mb-1 hover:bg-indigo-700 rounded-lg cursor-pointer ${adminMenu.name === 'pdSubmit' && 'bg-indigo-700'}`} onClick={() => handleMenuName('pdSubmit')}>
                    <a href="/admin/products/write" className='flex items-center'>
                        <RiPencilLine className='mr-2'/>상품 등록
                    </a>
                </li>
                <li className={`p-3 mb-1 hover:bg-indigo-700 rounded-lg cursor-pointer ${adminMenu.name === 'opSubmit' && 'bg-indigo-700'}`} onClick={() => handleMenuName('opSubmit')}>
