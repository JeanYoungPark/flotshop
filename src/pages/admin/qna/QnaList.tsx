    ...
    return (
        <div className='pt-10 flex w-full h-full justify-center items-center'>
            <div className='min-w-1/2'>
                <h1 className="text-xl font-semibold leading-10 text-gray-900">문의하기</h1>
                <ul className="divide-y divide-gray-100 mb-10">
                    {qna.map((qna) => (
                        <li key={qna.id} className="flex justify-between gap-x-6 py-5 ">
                            <div className="flex min-w-0 gap-x-4">
                                <img className="h-12 w-12 flex-none rounded-full bg-gray-50" src={qna.imageUrl} alt="" />
                                <div className="min-w-0 flex-auto"> 
                                <p className="text-sm font-semibold leading-6 text-gray-900">{qna.title}</p>
                                <p className="mt-1 truncate text-xs leading-5 text-gray-500">{qna.date}</p>
                                </div>
                            </div>
